...
// ---------- V2.0: 手动写入 SIM 本机号码 ----------
// 使用 AT+CPBS="ON" + AT+CPBW=1,"号码"
bool Sim::writePhoneNumber(const String& phone) {
  if (phone.length() < 5) {
    LOG("SIM", "写入号码失败：号码无效");
    return false;
  }
  ...
  String cmd;
  if (!SimDispatcher::sendCommand("AT+CPBS=\"ON\"", 3000, nullptr, false)) {
    LOG("SIM", "AT+CPBS=ON失败");
    return false;
  }

  cmd = "AT+CPBW=1,\"" + phone + "\"";
  if (!SimDispatcher::sendCommand(cmd.c_str(), 5000, nullptr, false)) {
    LOG("SIM", "AT+CPBW写入失败");
    return false;
  }
  ...
  s_phoneNum = phone;
  s_numberReady = true;
  LOG("SIM", "本机号码写入成功: %s", phone.c_str());
  return true;
}
